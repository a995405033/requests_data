...
def find_province(city):
    ...
    area_data = {
        ...
        '新疆': ['新疆', '新疆维吾尔自治区', '乌鲁木齐市', '伊犁州', '昌吉州', '石河子市', '哈密地区', '阿克苏地区', '巴音郭楞州', '喀什地区', '塔城地区', '克拉玛依市',
...
if __name__ == '__main__':
    ...
    session.headers.update(
        {'referer': "https://www.che168.com/china/a0_0msdgscncgpi1ltocsp2exf4-3x0/?pvareaid=102179"}
    )
    session.headers.update(
        {
            'cookie': "userarea=0; ahpvno=1; fvlid=1772759968332nwYnMHZGGSHW; Hm_lvt_d381ec2f88158113b9b76f14c497ed48=1772759969; Hm_lpvt_d381ec2f88158113b9b76f14c497ed48=1772759969; HMACCOUNT=276496E59BA2AEE2; sessionid=236f658f-c95d-4cd2-8bae-99ff8a54626a; sessionip=120.39.41.18; area=350104; sessionvisit=6f2d3442-95c0-47e4-be91-e6bee7b2ba06; sessionvisitInfo=236f658f-c95d-4cd2-8bae-99ff8a54626a||0; ahuuid=4A6CD50C-8C32-4A32-8BB4-08A142E6EDF2; showNum=1; che_sessionid=E1A989C4-4DCC-42C4-A2EB-9EEA9AE4C784%7C%7C2026-03-06+09%3A19%3A28.415%7C%7C0; v_no=1; visit_info_ad=E1A989C4-4DCC-42C4-A2EB-9EEA9AE4C784||91DC2486-9852-4974-82D5-C5F178385869||-1||-1||1; che_ref=0%7C0%7C0%7C0%7C2026-03-06+09%3A19%3A28.415%7C2026-03-06+09%3A19%3A28.415; che_sessionvid=91DC2486-9852-4974-82D5-C5F178385869; sessionuid=236f658f-c95d-4cd2-8bae-99ff8a54626a"}
    )
    ...
    for _ in range(2, page + 1):
        ...
        page = session.get(
            url=f"https://www.che168.com/china/a0_0msdgscncgpi1ltocsp{_}exf4-3x0/?pvareaid=102179#currengpostion", )
        ...
        for li in li_list:
            try:
                tag = ""
                name = li.xpath("./a/div[2]/h4/text()")[0]
                url = "https://www.che168.com/" + li.xpath("./a/@href")[0]
                price = li.xpath("./a/div[2]/div[1]/span[1]/em/text()")[0]
                desc_raw = str(li.xpath("./a/div[2]/p/text()")[0])
                use = str(desc_raw.split("／")[0]).strip().replace("万公里", "")  # 公里数
                sp_time = str(desc_raw.split("／")[1]).strip()  # 上牌时间
                adress = str(desc_raw.split("／")[2]).strip()  # 所在地
                province = find_province(adress)  # 省份
                level = str(desc_raw.split("／")[3]).strip()  # 用户等级
                fb_time = str(li.xpath("./@publicdate")[0])[0:10]  # 发布时间
                try:
                    tag_list = li.xpath("./a/div[2]/div[1]/span[2]/i/text()")[0]
                    for datas in tag_list:
                        tag = tag + datas
                except:
                    tag_list = "正常出售"

                print(name,url,price,desc_raw,use,sp_time)
            except:
                pass
